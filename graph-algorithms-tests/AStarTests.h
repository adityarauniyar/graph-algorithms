...
TEST(AStarTests, Stress_Test) {
	...
	for (llong test = 0; test < 1e4; test++) {
		...
		Adv::AStar As(n, AdjListWithCost, xy);
		llong AStarResult = As.query(u, v);

		// EXPECT_EQ(DijkstraResult, AStarResult);
...
TEST(AStarTests, CASE_4) {
	int n;
	Adj AdjListWithCost(2);
	vector<pair<llong, llong>> xy;
	std::vector<std::pair<int, int>> Queries;
	std::istringstream inputStrm("4 5 1 2 -3 4 2 1 -3 4 1 3 1 1 4 3 1 2 5 2 3 5 3 4 1 1 1 4");
	inputToVars(inputStrm, n, xy, AdjListWithCost, Queries);

	Adv::AStar As(n, AdjListWithCost, xy);
	string ActualResult = As.query(Queries);

	EXPECT_EQ("2", ActualResult);
}